...
export const SPLIT_NOTICES = Object.fromEntries(
  Object.entries(ACTIONS_RENDUES).map(([code, [name, shares]]) => {
    ...
    return [code, {
      ...
      reason: "Le marché $CKM rouvre, et l'action y vaut désormais 500 🍪 au lieu de 100. Pour que ce changement d'échelle ne crée pas de cookies à partir de rien, 5 anciennes actions valent désormais 1 nouvelle action. Tu en as donc moins — mais elles valent cinq fois plus.",
      gained: [
      ...
      footer: "Le cours ne bouge plus tout seul : il ne monte que si des joueurs achètent, il ne descend que si des joueurs vendent. À toi de jouer ☕",
    }];
  })
);
